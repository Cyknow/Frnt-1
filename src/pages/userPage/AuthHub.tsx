...
const AuthHub = () => {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // SIGNUP LOGIC: Do not log in. Require email verification.
      if (!isLogin) {
        setIsSuccess(true);
        setInfoMessage("Verification transmission initiated. Check your inbox to activate access.");
        setTimeout(() => {
            setIsSuccess(false);
    ...
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-slate-950 font-sans">
      ...
      <div className="container mx-auto px-6 relative z-20 grid lg:grid-cols-2 gap-12 items-center">
        ...
        >
          ...
          <h1 className="text-6xl font-black leading-tight mb-6 italic uppercase whitespace-pre-line">
            {isLogin ? "YOUR IMPACT \n CONTINUES." : "THE WORLD \n IS WAITING."}
          </h1>
        ...
        <div className="w-full max-w-md mx-auto">
          <motion.div layout className="bg-white/10 backdrop-blur-2xl border border-white/20 p-8 md:p-10 rounded-[2.5rem] shadow-2xl">
            ...
            {infoMessage && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl flex items-center gap-3 text-blue-400 text-xs font-bold uppercase tracking-widest">
                <Info size={18} />
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <div className={`p-3 rounded-xl border flex items-center justify-between ${isRobotChecked ? 'bg-green-500/10 border-green-500/30' : 'bg-black/20 border-white/5'}`}>
                      <div className="flex items-center gap-3">
                        <input type="checkbox" checked={isRobotChecked} onChange={(e) => setIsRobotChecked(e.target.checked)} className="accent-green-500" />
                ...
                <motion.button
                  ...
                  className={`w-full py-4 rounded-2xl font-black text-slate-950 flex items-center justify-center gap-2 mt-4 transition-all shadow-xl
                    ${(loading || (!isLogin && (!isRobotChecked || !agreedToTerms))) 
                      ...
                      : 'opacity-100 cursor-pointer hover:shadow-yellow-400/20'
                    }`}
                  style={{ background: theme.gold }}
                >
                  {loading ? (isLogin ? 'SECURED ACCESS...' : 'CREATING ACCOUNT...') : (isLogin ? 'SECURED ACCESS' : 'CREATE ACCOUNT')}
                  <ArrowRight size={20} />
                </motion.button>
              </motion.form>
            </AnimatePresence>
      ...
      {/* MODALS */}      
        {legalModal.show && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setLegalModal({ show: false, type: legalModal.type })}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-slate-900 border border-white/10 p-8 rounded-[2.5rem] max-w-2xl max-h-[80vh] overflow-y-auto text-white relative shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <button onClick={() => setLegalModal({ show: false, type: legalModal.type })} className="absolute top-6 right-6 text-slate-400 hover:text-white"><X size={24} /></button>
              <h2 className="text-2xl font-black mb-6 italic uppercase tracking-tighter">{legalModal.type === 'terms' ? 'Terms of Service' : 'Privacy Policy'}</h2>
              <div className="text-sm leading-relaxed space-y-4 text-slate-300">
                <h3 className="text-lg font-bold text-white">{activeContent.title}</h3>
                <p className="text-[10px] uppercase tracking-widest text-yellow-400">Last updated: {activeContent.lastUpdated}</p>
                {activeContent.sections.map((section, index) => (
                  <div key={index} className="mb-6">
                    <h4 className="font-bold text-white mb-2 uppercase text-xs tracking-widest">{section.heading}</h4>
                    <p>{section.body}</p>
                  </div>
                ))}
              </div>
        ...
        {/* SUCCESS OVERLAY (Find this at the bottom of AuthHub) */}
        <AnimatePresence>
          {isSuccess && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] bg-slate-950 flex flex-col items-center justify-center text-center">
              <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 2 }} className="w-24 h-24 bg-yellow-400 rounded-3xl flex items-center justify-center mb-6 shadow-2xl shadow-yellow-400/40">
              ...
              <p className="text-slate-500 mt-2 font-medium tracking-widest uppercase text-[10px] max-w-xs">
                  {isLogin 
                    ? "Syncing with global vanguard network..." 
                    : "Identity transmission successful. Redirecting to verification terminal..."}
              </p>
            </motion.div>
          )}
        </AnimatePresence>
{/* 
        {isSuccess && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] bg-slate-950 flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">
                {isLogin ? "Authenticated" : "Transmission Received"}
            </h2>
            <p className="text-slate-500 mt-2 font-medium tracking-widest uppercase text-[10px]">
                {isLogin ? "Syncing with global vanguard network..." : "Awaiting identity verification..."}
            </p>
          </motion.div>
        )}
       */}
