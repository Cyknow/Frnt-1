...
import heroImage from "../../assets/download (1).jpg";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Night Ops active from 6 PM (18) to 6 AM (6)
    if (hour >= 18 || hour < 6) {
      setIsNightOps(true);
    }
  }, []);
  ...
  return (
    <div className="bg-slate-950 text-white overflow-hidden relative">
      ...
      {/* 2. STAR LAYER - Only visible during Night Ops */}
      <AnimatePresence>
        {isNightOps && (
          <motion.div 
            initial={{ opacity: 0 }}
      ...
      <div className="relative z-10"> {/* Wrap content to stay above stars */}
        <CharityHero/>
        ...
        <section className="relative min-h-screen flex items-center">
          ...
          <div className="relative container mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
            <motion.div variants={fadeUp} initial="hidden" animate="visible">
              {/* Optional Night Ops Badge */}
              {isNightOps && (
                <motion.span 
                  initial={{ opacity: 0, x: -20 }} 
                  animate={{ opacity: 1, x: 0 }}
                  className="inline-block mb-4 px-3 py-1 rounded-full border border-yellow-400/30 text-yellow-400 text-[10px] font-bold tracking-widest uppercase bg-yellow-400/5 backdrop-blur-md"
                >
                  ✨ Night Ops Protocol Active
                </motion.span>
              )}
          ...
          <div className="container mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              ...
              <motion.div
                ...
                whileInView="visible"
                viewport={{ once: true }}
                className="p-6 rounded-2xl bg-white/5 border border-white/5 hover:border-yellow-400/20 transition-colors backdrop-blur-sm"
              >
