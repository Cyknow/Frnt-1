...
const pillars = [
  ...
  },
  {
    ...
    desc: "By leveraging local networks and technology, we maintain a 92% efficiency rating, minimizing administrative costs."
  },
  {
...
const WhyChooseUs = () => {
  ...
  // 1. GENERATE HIGH-VISIBILITY STARS WITH DIRECTIONAL MOVEMENT
  const [stars] = useState(() => 
    Array.from({ length: 50 }).map((_, i) => ({
      id: i,
      // Sizes: Small (1-2px), Medium (3-4px), Large (5-6px)
      size: Math.random() * 5 + 1, 
      x: Math.random() * 100,
      y: Math.random() * 100,
      // Random movement offsets for different directions
      moveX: (Math.random() - 0.5) * 100, // Moves left or right
      moveY: (Math.random() - 0.5) * 100, // Moves up or down
      duration: Math.random() * 10 + 10,  // Slow drift
  ...
  return (
    <div className="relative bg-slate-950 text-white min-h-screen overflow-hidden">
            ...
            {stars.map((star) => (
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: `0 0 ${star.size * 2}px white, 0 0 ${star.size * 4}px rgba(255,255,255,0.3)`,
                }}
                animate={{
                  // Twinkle effect
                  opacity: [0.3, 1, 0.3],
      ...
      {/* 3. CONTENT (Relative Z-10 to sit above stars) */}
      <div className="relative z-10">
        {/* HERO SECTION */}
        <section className="relative py-24 overflow-hidden">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_center,rgba(30,58,138,0.2),transparent_70%)] -z-10" />
          ...
          <div className="container mx-auto px-6 text-center">
            {isNightOps && (
              <motion.div 
                ...
                className="inline-block mb-4 px-3 py-1 rounded-full border border-yellow-400/30 text-yellow-400 text-[10px] font-bold tracking-widest bg-yellow-400/10 backdrop-blur-md"
              >
                ✨ Night-Vision Active
              </motion.div>
            )}
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {pillars.map((pillar, index) => (
                <motion.div
                  ...
                  className="p-8 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-md transition-all duration-300 group"
                >
                  <div 
        ...
        <section className="py-20">
          <div className="container mx-auto px-6">
            <div className="bg-slate-900/80 backdrop-blur-2xl rounded-[3rem] p-8 md:p-16 border border-white/10 shadow-2xl relative overflow-hidden">
              <div className="grid lg:grid-cols-2 gap-12 items-center">
                <div className="relative z-10">
                  <h2 className="text-3xl font-bold mb-6 text-white">Redefining Aid</h2>
                  <div className="space-y-6">
                    {[
                      ...
                      <div key={i} className="flex items-start gap-4">
                        <div className="mt-1 bg-yellow-400 rounded-full p-1 text-slate-950 shadow-[0_0_10px_rgba(250,204,21,0.5)]">
                ...
                <div className="relative h-64 md:h-80 flex items-center justify-center">
                   <motion.div 
                     animate={{ 
                       scale: [1, 1.05, 1],
                       boxShadow: ["0 0 20px rgba(250,204,21,0.1)", "0 0 40px rgba(250,204,21,0.3)", "0 0 20px rgba(250,204,21,0.1)"] 
                     }}
                     transition={{ duration: 4, repeat: Infinity }}
                     className="text-center p-10 bg-black/60 backdrop-blur-xl rounded-[2rem] border border-yellow-400/20"
                   >
                      <span className="text-yellow-400 text-6xl font-black italic tracking-tighter">92%</span>
                      <p className="text-white font-bold mt-2 uppercase tracking-[0.2em] text-xs">Efficiency Rating</p>
                   </motion.div>
                </div>
              </div>
            </div>
        ...
        <section className="py-24 text-center">
          <div className="container mx-auto px-6">
            <h2 className="text-3xl font-bold text-white mb-4">Ready to make a real difference?</h2>
            <div className="w-24 h-1 bg-yellow-400 mx-auto rounded-full mb-8 shadow-[0_0_10px_#facc15]" />
          </div>
        </section>
      </div>
    </div>
